...
export default function ShowcaseSection() {
  ...
  return (
    <section ref={sectionRef} id="features" className="relative scroll-mt-24">
      ...
      <div className="overflow-hidden">
        <div
          ref={trackRef}
          className="flex gap-8 pl-[5vw] pr-[5vw] pb-16 pt-4"
          style={{ width: `${screens.length * 400 + 200}px` }}
        >
          {screens.map((screen, i) => (
            <div key={i} className="showcase-card flex-shrink-0 w-[360px] flex flex-col items-center">
              <PhoneMockup glowColor={screen.glowColor} className="scale-90 origin-top">
              <div className="absolute inset-0 bg-gradient-to-b from-naija-green-dark/50 via-surface-dark to-surface-dark flex flex-col p-5 pt-12">
                <div className="text-center mb-4">
                  <p className="text-[10px] text-naija-gold uppercase tracking-widest">{screen.title}</p>
                </div>
                ...
                <div className="mt-3">
                  <div className="h-1 bg-white/10 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-naija-green to-naija-gold rounded-full" style={{ width: `${(i + 1) * 20}%` }} />
                  </div>
                </div>
              </div>
              </PhoneMockup>
              <div className="mt-2 text-center max-w-xs px-4">
                <h3 className="text-lg font-heading font-bold text-text-primary">{screen.title}</h3>
                <p className="mt-1 text-xs text-text-secondary leading-relaxed">{screen.description}</p>
              </div>
            </div>
          ))}
        </div>
